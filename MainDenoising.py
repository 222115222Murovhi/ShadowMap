...
from corruption import MakeNoise
...
trainer = tr.Trainer(
    ...
    corruption=MakeNoise(corruption_rate=0.2, seed=123),
)
# Separate folder for each run, so previous results aren't overwritten.
run_dir = Path("runs") / datetime.now().strftime("Denoising_ae_0.2_%Y%m%d_%H%M%S")
